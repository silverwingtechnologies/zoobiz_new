...
jQuery($=> {
   ...
   $("#reachUsFrm").on('submit', function () {
    if($('#reachUsFrm').valid() == true){
        ...
        if (proceed) {
            ...
            $.ajax({
                ...
                cache: false,
                type: "POST",
                data: {mobile : mobile, userName : userName,userEmail:userEmail,userMessage:userMessage,feedback:'feedback'},
            ...
            // $.post('controller/contactController.php', postData, function (response) {
            //     //load json data from server and output message
            //          output = '<div class="alert-success" style="padding:10px; margin-bottom:25px;">Please provide the missing fields.</div>';

            //     // if (response.type === 'error') {
            //     //     output = '<div class="alert-danger" style="padding:10px; margin-bottom:25px;">' + response.text + '</div>';
            //     // } else {
            //     //     output = '<div class="alert-success" style="padding:10px; margin-bottom:25px;">Hello' + response.text + '</div>';
    ...
    $("#subscribeFrm").on('submit', function () {
        if($('#subscribeFrm').valid() == true){
        let userName = $('#name1').val();
        let userEmail = $('#email1').val();
        let userMessage = $('#message1').val();
        ...
        if (proceed) {
            ...
            $.ajax({
                ...
                cache: false,
                type: "POST",
                data: {subName:subName,subEmail:subEmail,subMobile:subMobile,subs : 'subs'},
                success: function(response){
                    $('#subName').val('');
                    $('#subEmail').val('');
                    $('#subMobile').val('');
