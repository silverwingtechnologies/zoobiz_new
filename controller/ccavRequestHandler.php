...
if($amount_to_be_paid > 0  ){
   $_POST['amount'] = $amount_to_be_paid;
    $merchant_data='';
...
$payment_getway_master_qry=$d->select("payment_getway_master,currency_master"," payment_getway_master.company_id ='$company_id' AND payment_getway_master.currency_id=currency_master.currency_id and payment_getway_master.ccav_working_key !=''  ","");
 if (mysqli_num_rows($payment_getway_master_qry) > 0 ) {
   ...
   $_POST['working_key'] =  $working_key=$payment_getway_master_data['ccav_working_key'];//Shared by CCAVENUES
 $_POST['access_code'] =    $access_code=$payment_getway_master_data['ccav_access_code'];
    $_POST['merchant_id']=$payment_getway_master_data['ccav_merchant_id'];

    if($payment_getway_master_data['ccav_live_mode']=="Yes"){
        $ccav_live_mode_url = "https://secure.ccavenue.com/transaction/transaction.do?command=initiateTransaction";
    } else {
        $ccav_live_mode_url = "https://test.ccavenue.com/transaction/transaction.do?command=initiateTransaction";
    }
 } else {
   $_POST['working_key'] =  $working_key='CF33EDAC825B5B39BAFB5CD2C955F90B';//Shared by CCAVENUES
   $_POST['access_code'] =  $access_code='AVGK04IC24AQ98KGQA';//Shared by CCAVENUES
    $_POST['merchant_id'] = '345038';
    $ccav_live_mode_url = "https://test.ccavenue.com/transaction/transaction.do?command=initiateTransaction";
 }
 
unset($_SESSION['working_key']);
unset($_SESSION['tracking_id']);
$_SESSION['working_key'] =$working_key;
$_SESSION['tracking_id'] =$_POST['tid'];	
	...
	<form method="post" name="redirect" action="<?php echo $ccav_live_mode_url;?>"> 
		<?php
