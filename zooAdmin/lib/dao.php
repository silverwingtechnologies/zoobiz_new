...
{    
    ...
    //11march21
    function plan_days_left($plan_renewal_date){
      $curDate = date("Y-m-d");
      $start_date = strtotime($curDate); 
      $end_date = strtotime($plan_renewal_date); 
  ...
  //NEW SMS CODE
    ...
    function sms_reminder3($mobile,$sec) {
     ...
     function sms_admin_reset_password($mobile,$full_name,$forgotLink) {
         $forgotLink=urlencode($forgotLink);
        $sms= file_get_contents("https://2factor.in/API/R1/?module=TRANS_SMS&apikey=2eb6de0f-3a58-11e9-8806-0200cd936042&to=$mobile&from=ZOOBIZ&templatename=Reset+Password+-+Admin+Panel&var1=$full_name&var2=$forgotLink");
    }
  ...
  function sms_to_admin_on_new_user_registration($mobile,$uname,$city_name,$ref_by_data) {
    ...
    function sms_to_member_on_registration($mobile,$member_name,$alink=NULL,$ilink=NULL) {
         $member_name=urlencode($member_name); 
         $alink=urlencode($alink);
...
//employment details per end
 ...
 //echo $add_per_val.'+'.$user_per_val.'+'.$user_emp_per_val;exit;
    ...
    //23dec2020
    function insert_myactivity($app_user_id,$user_id,$user_name,$log_name,$log_img)
    {   
      $log_name = $this->conn->real_escape_string($log_name);
      $user_name = $this->conn->real_escape_string($user_name);
        $now=date("y-m-d H:i:s");
        $val="'$app_user_id','$user_id','$user_name','$log_name','$now','$log_img'";
        mysqli_set_charset($this->conn,"utf8mb4");
        return mysqli_query($this->conn,"INSERT INTO log_master(app_user_id,user_id,user_name,log_name,log_time,log_img) VALUES($val)") or die(mysqli_error($this->conn));
    }

    function insert_cron_log($log_name,$type)
    {   
      $log_name = $this->conn->real_escape_string($log_name);
      
        $now=date("Y-m-d H:i:s");
        $val="'$log_name','$now','$type'";
        mysqli_set_charset($this->conn,"utf8mb4");
        return mysqli_query($this->conn,"INSERT INTO cron_log_master(log_name,log_time,type) VALUES($val)") or die(mysqli_error($this->conn));
    }
    //
//23dec2020
...
$blocked_users = implode(",", $blocked_users); 
    ...
    //17SEPT2020
//17march21
    function insertAllUserNotificationMemberSpecial($title,$description,$notification_action,$notification_icon=null,$append_query=null,$notification_type=0,$other_user_id=0)
    { 
       ...
       date_default_timezone_set('Asia/Calcutta');
        ...
        $description = $this->conn->real_escape_string($description);
        mysqli_set_charset($this->conn,"utf8mb4");
     return mysqli_query($this->conn,"INSERT INTO user_notification(user_id,notification_title,notification_desc,notification_date,notification_action,notification_logo,notification_type,other_user_id) SELECT   user_id,'$title','$description','$today','$notification_action','$notification_icon','$notification_type','$other_user_id' FROM users_master   WHERE  active_status = 0 $append_query ") or die(mysqli_error($this->conn));
    }
    //17march21
    //10feb2021
     function insertAllUserNotificationToIos($title,$description,$notification_action,$notification_icon=null,$append_query=null,$notification_type=0)
    { 
       ...
       date_default_timezone_set('Asia/Calcutta');
        ...
        $description = $this->conn->real_escape_string($description);
        mysqli_set_charset($this->conn,"utf8mb4");
     return mysqli_query($this->conn,"INSERT INTO user_notification(user_id,notification_title,notification_desc,notification_date,notification_action,notification_logo,notification_type) SELECT   user_id,'$title','$description','$today','$notification_action','$notification_icon','$notification_type' FROM users_master   WHERE  active_status = 0 AND lower(device)='ios' $append_query ") or die(mysqli_error($this->conn));
    }

     function insertAllUserNotificationToAndroid($title,$description,$notification_action,$notification_icon=null,$append_query=null,$notification_type=0)
    { 
       ...
       date_default_timezone_set('Asia/Calcutta');
        ...
        $description = $this->conn->real_escape_string($description);
        mysqli_set_charset($this->conn,"utf8mb4");
     return mysqli_query($this->conn,"INSERT INTO user_notification(user_id,notification_title,notification_desc,notification_date,notification_action,notification_logo,notification_type) SELECT   user_id,'$title','$description','$today','$notification_action','$notification_icon','$notification_type' FROM users_master   WHERE  active_status = 0 AND lower(device)='android' $append_query ") or die(mysqli_error($this->conn));
    }
    //10feb2021

 function insertAllUserNotificationTimeline($title,$description,$notification_action,$notification_icon=null,$append_query=null,$notification_type=0,$timeline_id=0)
    { 
       ...
       date_default_timezone_set('Asia/Calcutta');
        ...
        $description = $this->conn->real_escape_string($description);
        mysqli_set_charset($this->conn,"utf8mb4");
     return mysqli_query($this->conn,"INSERT INTO user_notification(user_id,notification_title,notification_desc,notification_date,notification_action,notification_logo,notification_type,timeline_id) SELECT   user_id,'$title','$description','$today','$notification_action','$notification_icon','$notification_type','$timeline_id' FROM users_master   WHERE  active_status = 0 $append_query ") or die(mysqli_error($this->conn));
    }
 ...
 function last_auto_id_temp($table)
    {
        mysqli_set_charset($this->conn,"utf8mb4");
//get primary field name
        $primary_key_query = mysqli_query($this->conn,"SELECT COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_NAME = '$table'   AND CONSTRAINT_NAME = 'PRIMARY'  " ) or die(mysqli_error($this->conn));
     $primary_key_data=mysqli_fetch_array($primary_key_query);
     $field_name = $primary_key_data['COLUMN_NAME'];
 
     $select_id=$this->selectRow(" MAX($field_name)+1 AS Auto_increment ",$table,"","");
...
//31dec2020
    function get_settings_details(){
        $select = mysqli_query($this->conn,"SELECT sms_api_link,otp_api_link,multiple_sms_link FROM zoobiz_settings_master where setting_id=1") or die(mysqli_error($this->conn));
         $data = mysqli_fetch_array($select);
         return $data;
    }
//31dec2020
...
// sms send 
   function send_sms($mobile,$message) {
      ...
      /* $msg=urlencode($message);
       ...
       $settings_data = $this->get_settings_details();
       if(count($settings_data) > 0){
        $sms_url = htmlspecialchars_decode ($settings_data['sms_api_link']);
        $sms_url =  str_replace("CELL_PHONE_NUMBER",$mobile,$sms_url);
        $sms_url =  str_replace("SMS_CONTENT",$msg,$sms_url);
     ...
     function send_sms_temp($mobile,$message) {
    ...
    function send_otp($mobile,$otp) {
      ...
      // $sms= file_get_contents("https://2factor.in/API/V1/2eb6de0f-3a58-11e9-8806-0200cd936042/SMS/$mobile/$otp/ZooBiz");
        $settings_data = $this->get_settings_details();
       if(count($settings_data) > 0){
        $otp_url = htmlspecialchars_decode ($settings_data['otp_api_link']);
        $otp_url =  str_replace("CELL_PHONE_NUMBER",$mobile,$otp_url);
        $otp_url =  str_replace("OTP_MSG",$otp,$otp_url);

        $sms= file_get_contents($otp_url);
       } else {
        $sms= file_get_contents("https://2factor.in/API/V1/2eb6de0f-3a58-11e9-8806-0200cd936042/SMS/$mobile/$otp/ZooBiz");
       }
       //31dec2020
  ...
  function updateCounter($table ,$value='')
    ...
    function send_sms_multiple($mobiles,$message) {
      ...
      //31dec2020
      $curl_url = "http://2factor.in/API/V1/2eb6de0f-3a58-11e9-8806-0200cd936042/ADDON_SERVICES/SEND/TSMS";
       $settings_data = $this->get_settings_details();
       if(count($settings_data) > 0){
        $curl_url = htmlspecialchars_decode ($settings_data['multiple_sms_link']);
       }  
       //31dec2020
