...
if(strtotime($data_notification['created_date']) < strtotime('-30 days')) {
			...
			if ($feed_type == "1") {
				...
				if ($d == TRUE) {
					...
					$d->insertFollowNotification($title, $description, $timeline_id, $user_id, 1, "timline");
					...
					$fcmArrayAndroidFollow = $d->get_android_fcm("users_master,category_follow_master", "users_master.user_id=category_follow_master.user_id AND category_follow_master.category_id='$getCatData[business_category_id]' AND users_master.user_token!='' AND  users_master.device='android' ");

					$fcmArrayIosFollow = $d->get_android_fcm("users_master,category_follow_master", "users_master.user_id=category_follow_master.user_id AND category_follow_master.category_id='$getCatData[business_category_id]' AND users_master.user_token!='' AND  users_master.device='ios' ");

					$fcmArray = array_merge($fcmArrayAndroidFollow, $fcmArray);
					$fcmArrayIos = array_merge($fcmArrayIosFollow, $fcmArrayIos);

					$nResident->noti("timeline", "", 0, $fcmArray, $title, $description, 'timeline');
					$nResident->noti_ios("TimelineVC", "", 0, $fcmArrayIos, $title, $description, 'timeline');
			...
			if ($like_status == 0) {
				...
				if (mysqli_num_rows($qcheck) > 0) {
					...
					if ($feed_user_id != $user_id && $feed_user_id != '0' AND $feed_user_id != '') {
						...
						if ($device == 'android') {
							$nResident->noti("", "", $society_id, $sos_user_token, $title, $msg, 'timeline');
						} else if ($device == 'ios') {
							$nResident->noti_ios("", "", $society_id, $sos_user_token, $title, $msg, 'timeline');
						}
					}
				}
			...
			$qnotification = $d->select("timeline_master,users_master,user_employment_details", "user_employment_details.user_id=users_master.user_id AND timeline_master.active_status = 0 AND timeline_master.user_id=users_master.user_id   AND timeline_master.timeline_id='$timeline_id'", "");
