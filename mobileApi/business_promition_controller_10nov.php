...
$promotion_master=$d->select("promotion_master","   status = 0 order by event_date asc   ","");
	...
	if(mysqli_num_rows($promotion_rel_frame_master)  > 0    ){
		...
		while ($promotion_rel_frame_master_data = mysqli_fetch_array($promotion_rel_frame_master)) {
			...
			$frameDetails["event_frame"] =$event_frame;
 			...
 			$frameDetails["event_status"] =$event_status;
...
$today = date("Y-m-d");
// /event_date>= '$today' and
$promotion_master=$d->select("promotion_master","  status = 0 and promotion_id ='$promotion_id' order by event_date asc  ","");
if(mysqli_num_rows($promotion_master)  > 0    ){
