	...
	if ($key == $keydb) {
		...
		if ($_POST['getCllassified'] == "getCllassified" && filter_var($user_id, FILTER_VALIDATE_INT) == true) {
			...
			if ($q1 > 0) {
				...
				$fcmArray = $d->get_android_fcm("users_master", "user_id NOT IN ('$ids') AND cllassified_mute=0 AND user_id='$comment_user_id' AND user_token!='' AND  device='android' AND user_id!='$user_id'");
				$fcmArrayIos = $d->get_android_fcm("users_master", "user_id NOT IN ('$ids') AND cllassified_mute=0 AND user_id='$comment_user_id'  AND user_token!='' AND  device='ios' AND user_id!='$user_id'");
