...
if (isset($_POST) && !empty($_POST)) {
	...
	if ($key == $keydb) {
		...
		if ($_POST['getCllassifiedCategories'] == "getCllassifiedCategories" ) {
			...
			$q = $d->select("cllassifieds_master,business_categories", "cllassifieds_master.business_category_id  =business_categories.business_category_id and business_categories.category_status = 0  ", "GROUP BY business_categories.business_category_id ORDER BY business_categories.category_name ASC ");
           ...
           if (mysqli_num_rows($q) > 0) {
				...
				while ($data = mysqli_fetch_array($q)) {
					...
					$totalCls = $d->count_data_direct("cllassified_id","cllassifieds_master", "business_category_id='$business_category_id'  ");
					$discussion = array();
			...
			$q = $d->select("cllassifieds_master,business_sub_categories", "cllassifieds_master.business_sub_category_id  =business_sub_categories.business_sub_category_id and business_sub_categories.business_category_id='$business_category_id'  ", "GROUP BY cllassifieds_master.business_sub_category_id ORDER BY business_sub_categories.sub_category_name ASC ");
           ...
           if (mysqli_num_rows($q) > 0) {
				...
				while ($data = mysqli_fetch_array($q)) {
					...
					$totalClsSub = $d->count_data_direct("cllassified_id","cllassifieds_master", "business_sub_category_id='$business_sub_category_id'  ");
					$discussion = array();
					$discussion["calssified_sub_cat_entries"] = $totalClsSub;
					$discussion["business_sub_category_id"] = $data['business_sub_category_id'];
					$discussion["business_category_id"] = $data['business_category_id'];
					$discussion["sub_category_name"] = html_entity_decode($data['sub_category_name']);
		...
		} else if ($_POST['getCllassifiedFilter'] == "getCllassifiedFilter" && filter_var($user_id, FILTER_VALIDATE_INT) == true) {
			...
			if (mysqli_num_rows($q) > 0) {
				...
				while ($data = mysqli_fetch_array($q)) {
					...
					$discussion = array();
					$discussion["appendQuery"] = $appendQuery;
		...
		if ($_POST['getCllassified'] == "getCllassified" && filter_var($user_id, FILTER_VALIDATE_INT) == true) {
			...
			// }else {
			$q = $d->select("cllassifieds_master,cllassifieds_city_master", "cllassifieds_master.active_status=0  AND cllassifieds_master.cllassified_id=cllassifieds_city_master.cllassified_id AND $appendQuery", "GROUP BY cllassifieds_master.cllassified_id ORDER BY cllassifieds_master.cllassified_id DESC ");
			// }
