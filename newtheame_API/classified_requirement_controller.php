...
$subCatArr = array('0');
   $fi1333 = $d->selectRow(
    ...
    "cllassifieds_master,business_sub_categories",
    "business_sub_categories.business_sub_category_id=cllassifieds_master.business_sub_category_id AND  cllassifieds_master.cllassified_id='$data[cllassified_id]' "
    );
    while ($feeData133 = mysqli_fetch_array($fi1333)) {
    $sub_category_array = array();
    if(!in_array($feeData133['business_sub_category_id'], $subCatArr)){
        $subCatArr[] = $feeData133['business_sub_category_id'];
        $sub_category_array["business_sub_category_id"] = $feeData133['business_sub_category_id'];
    $sub_category_array["sub_category_name"] = $feeData133['sub_category_name'];
    ...
    $sub_category_array = array();
     ...
     if(!in_array($feeData1['business_sub_category_id'], $subCatArr)){
        $subCatArr[] = $feeData1['business_sub_category_id'];
    ...
    }  else if ($_POST['getCllassifiedSearchCityandCat'] == "getCllassifiedSearchCityandCat" && filter_var($user_id, FILTER_VALIDATE_INT) == true) {
    $blocked_users = array('0');
    $getBLockUserQry = $d->selectRow("user_id, block_by", "user_block_master", " block_by='$user_id' or user_id='$user_id'  ", "");
    while ($getBLockUserData = mysqli_fetch_array($getBLockUserQry)) {
      if ($user_id != $getBLockUserData['user_id']) {
    ...
    $searchCityId  =$city_id;
    $searchCatId  =$business_category_id ;
    $searchSubCatId  =$business_sub_category_id ;
   /* if (trim($search) != '') {
    $search = trim($search);
    $query2 .= " and  ( c.cllassified_title LIKE '%$search%'    or c.cllassified_description LIKE '%$search%'  or users_master.user_full_name LIKE '%$search%'  or (business_categories.business_category_id = cc.business_category_id  or  business_sub_categories.business_sub_category_id = cc.business_sub_category_id) or (cities.city_id = cm.city_id )  ) ";
    $catSearch = " and  (  business_categories.business_category_id IN (business_category_id)  ) ";
    $subCatSearch = " and  (  business_sub_categories.business_sub_category_id IN (business_sub_category_id)  ) ";
    $citiesSearch = " and  (  cities.city_id LIKE IN (city_id)  ) ";
    }*/

     if (trim($city_id) != '') {
   //   $query2 .= " ";
     $query2 = " and  (  cities.city_id  IN ($searchCityId)  ) ";
    }

     if (trim($business_category_id) != '') {
    //  $query2 .= " ";
       $query2 = " and  (  business_categories.business_category_id IN ($searchCatId)  ) ";
    }

     if (trim($business_sub_category_id) != '') {
   //   $query2 .= " ";

       $query2 = " and  (  business_sub_categories.business_sub_category_id IN ($searchSubCatId)  ) ";
