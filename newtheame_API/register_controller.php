...
//echo "<pre>";print_r($_REQUEST);exit;
	...
	if (isset($user_register) && $user_register == 'user_register' && filter_var($userMobile, FILTER_VALIDATE_INT) == true  /*&& filter_var($userMobile, FILTER_VALIDATE_INT) == true*/) {
...
if(isset($amount)){
	...
	$ios_transection_amount = "0.00";
		...
		$q = $d->selectRow("package_name,no_of_month,package_amount,package_amount,gst_slab_id","package_master", "package_id='$plan_id'", "");
		...
		if(isset($coupon_code) && $coupon_code !=""){ 
			$coupon_master=$d->selectRow("package_master.package_amount,coupon_master.coupon_amount,coupon_master.coupon_per","coupon_master, package_master","  package_master.package_id =coupon_master.plan_id and  coupon_master.coupon_code ='$coupon_code' and coupon_master.coupon_status = 0    ","");
...
//8march21
		...
		//3nov 2020
			...
			if ($q and $q3 and $q4) {
				...
				$ref_by_data ="";
				//refer by user start
				$main_users_master = $d->selectRow("user_id,refer_by,refere_by_phone_number,refere_by_name,   user_profile_pic,user_full_name,remark","users_master", "user_mobile = '$user_mobile' OR user_mobile ='$userMobile'    ");
				...
				if($main_users_master_data['refer_by']==2){ 
					...
					if (mysqli_num_rows($ref_users_master) > 0) {
						...
						if($ref_users_master_data['user_token'] !=''){
							...
							//ucfirst($main_users_master_data['user_full_name']).", is now a proud member of ZooBiz ".$cities_data['city_name']." region. Thank you ".ucfirst($ref_users_master_data['user_full_name'])." for referring.";
							if (strtolower($ref_users_master_data['device']) =='android') {
								/*$nResident->noti("viewMemeber","",0,$ref_users_master_data['user_token'],$title,$msg3,$main_users_master_data['user_id'],1,$profile_u);*/
								 ...
								 $nResident->noti("refer",$profile_u,0,$ref_users_master_data['user_token'],$title,$msg3,"0",1,$profile_u);
								...
								$nResident->noti_ios("refer",$profile_u,0,$ref_users_master_data['user_token'],$title,$msg3,"0",1,$profile_u);
						...
						//$d->send_sms($ref_users_master_data['user_mobile'], $msg3);
						...
						//ucfirst($main_users_master_data['user_full_name']).", is now a proud member of ZooBiz ".$cities_data['city_name']." region. Thank you ".ucfirst($main_users_master_data['refere_by_name'])." for referring.";
						//$d->send_sms($main_users_master_data['refere_by_phone_number'], $msg3);
...
} else if (isset($user_register_temp) && $user_register_temp == 'user_register_temp' && filter_var($userMobile, FILTER_VALIDATE_INT) == true) {
...
$user_full_name = ucfirst($user_first_name) .' '.ucfirst($user_last_name);
	...
	if(isset($coupon_code) && $coupon_code !='' && isset($apply_coupon) && $apply_coupon    ){
		...
		$coupon_master=$d->selectRow("coupon_master.coupon_id,package_master.package_amount,coupon_master.coupon_amount,coupon_master.coupon_per","coupon_master, package_master","  package_master.package_id =coupon_master.plan_id and  coupon_master.coupon_code ='$coupon_code' and coupon_master.coupon_status = 0    ","");
	...
	if ($q > 0) {
		...
		if (isset($city_id)) {
			...
			$payment_getway_master_qry = $d->selectRow("company_id,currency_id,merchant_id,merchant_key","payment_getway_master", " company_id ='$company_id' ", "");
			...
			if (mysqli_num_rows($payment_getway_master_qry) > 0) {
				...
				$keyId = 'rzp_live_hTssMmy74UFuJp';
				$keySecret = 'K6rCFG25oUGmMVkgNNLonzAM';
				$displayCurrency = 'INR';
			}
		} else {
			$keyId = 'rzp_live_hTssMmy74UFuJp';
	...
	if ($q > 0) {
		...
		if (isset($city_id)) {
			...
			if ($q and $q1 and $q2) {
				...
				$users_master_qry = $d->selectRow("*","users_master", " user_id='$user_id'", "");
